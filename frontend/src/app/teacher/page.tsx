...
const quickTestPhrases = [
  { label: "Enviar frase de Matemática", text: "Hoje vamos aprender o conceito de soma e divisão." },
  { label: "Enviar frase de Ciências", text: "A água muda de estado físico com a temperatura." },
...
export default function TeacherPage() {
  ...
  async function generateLiveSummaryNow() {
    setError(null);
    if (!classSession) {
      setError("Crie uma aula antes de gerar resumo.");
      return;
    }
    setSummaryLoading(true);
    try {
      const mapped = await createLiveSummary(classSession.id);
      setLiveSummary(mapped);
      setSummary(mapped.summaryText);
      setFeedback("Resumo automático atualizado.");
    } catch {
      setError("Não foi possível gerar o resumo agora. Verifique a conexão com a API.");
    } finally {
      setSummaryLoading(false);
    }
  }

  async function copyLiveSummary() {
    const text = liveSummary?.summaryText ?? summary;
    if (!text) return;
  ...
  async function sendTranscriptToStudents(text: string) {
    setError(null);
    if (!classSession) {
      setError("Crie uma aula antes de enviar trechos para os alunos.");
      return false;
    }
    if (!text.trim()) {
      setError("Digite ou escolha uma frase para enviar.");
      return false;
    }
    try {
      await sendTranscript(classSession.id, text.trim());
      setTranscript((current) => [text.trim(), ...current].slice(0, 16));
      setFeedback("Trecho enviado para os alunos.");
      return true;
    } catch {
      setError("Não foi possível enviar a transcrição. Verifique a conexão com a API ou faça login novamente.");
      return false;
    }
  }
    ...
    <main className="min-h-screen bg-paper dark:bg-zinc-950">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[390px_1fr]">
        ...
        <section className="space-y-4">
          ...
          <section className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-white/10 dark:bg-zinc-900">
            <h2 className="text-xl font-black text-ink dark:text-white">Teste rápido da aula</h2>
            <p className="mt-1 text-sm font-semibold text-ink/65 dark:text-white/65">
              Envie frases curtas para validar legenda, cards e avatar/fallback em outra aba ou no celular.
            </p>
            <div className="mt-4 grid gap-2 md:grid-cols-3">
              {quickTestPhrases.map((item) => (
                <ActionButton
                  key={item.label}
                  tone="quiet"
                  onClick={() => sendTranscriptToStudents(item.text)}
          ...
          <section className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-white/10 dark:bg-zinc-900">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-ocean dark:text-mint">
                  {liveSummary?.generatedBy === "ai" ? "IA ativa" : "Resumo local ativo"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <ActionButton tone="quiet" onClick={generateLiveSummaryNow} disabled={!classSession || summaryLoading}>
                  {summaryLoading ? "Gerando..." : "Gerar resumo agora"}
                </ActionButton>
                <ActionButton tone="secondary" onClick={copyLiveSummary} disabled={!liveSummary && !summary}>
                  Copiar resumo
                </ActionButton>
              </div>
            </div>
            <p className="mt-3 rounded-lg bg-amber/20 p-3 text-xs font-bold leading-relaxed text-ink dark:text-white">
              Resumo gerado automaticamente com base na transcrição. Revise antes de usar como material oficial.
            </p>
          </section>

          <LiveSummaryPanel
            summaryText={liveSummary?.summaryText ?? summary ?? undefined}
            bulletPoints={liveSummary?.bulletPoints}
            keywords={liveSummary?.keywords}
            updatedAt={liveSummary?.updatedAt}
            generatedBy={liveSummary?.generatedBy}
            isAutoGenerated={liveSummary?.isAutoGenerated}
