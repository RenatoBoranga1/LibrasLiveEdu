...
from app.schemas.api import ClassSessionCreate, ClassSessionPublicRead, ClassSessionRead, LiveSummaryEventRead, TranscriptInput
...
async def finish_class(
    ...
    live_summary = LiveLessonSummaryService(db).generate_live_summary(class_session_id)
    db.commit()
    await websocket_manager.broadcast(
        class_session.access_code,
        "summary.created",
        {"summaryText": live_summary["summary_text"], "keywords": live_summary["keywords"]},
    )
    await websocket_manager.broadcast(
        class_session.access_code,
        "summary.updated",
        _summary_event_payload(live_summary),
    )
    await websocket_manager.broadcast(class_session.access_code, "class.finished", {"id": class_session.id})
    return {"status": class_session.status, "summary": live_summary["summary_text"], "keywords": live_summary["keywords"]}
...
@router.post("/{class_session_id}/summary/live", response_model=LiveSummaryEventRead)
async def create_live_summary(
    class_session_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(require_role(["professor", "admin"])),
):
    ...
    if not class_session:
        raise HTTPException(status_code=404, detail="Aula nao encontrada.")
    _ensure_teacher_owner_or_admin(class_session, user)
    result = LiveLessonSummaryService(db).generate_live_summary(class_session_id)
    db.commit()
    payload = _summary_event_payload(result)
    await websocket_manager.broadcast(class_session.access_code, "summary.updated", payload)
    return payload


@router.get("/access/{access_code}/summary/live", response_model=LiveSummaryEventRead)
def get_live_summary_by_access_code(access_code: str, db: Session = Depends(get_db)):
    class_session = db.scalar(select(ClassSession).where(ClassSession.access_code == access_code.upper()))
    if not class_session:
        raise HTTPException(status_code=404, detail="Aula nao encontrada.")
    service = LiveLessonSummaryService(db)
    result = service.latest_summary(class_session.id)
    if not result:
        return {
            "summaryText": "",
            "bulletPoints": [],
    ...
    return _summary_event_payload(result)
...
def _summary_event_payload(summary: dict) -> dict:
