...
export default function ReviewByCodePage() {
  ...
  const [review, setReview] = useState<ClassReview>(fallbackReview);

  useEffect(() => {
    getReviewByAccessCode(accessCode).then(setReview).catch(() => setReview({
      ...fallbackReview,
      classSession: { ...fallbackReview.classSession, access_code: accessCode },
    }));
  }, [accessCode]);

  const summary = `Resumo demo: a aula "${review.classSession.title}" reuniu os principais termos ${review.keywords.slice(0, 5).join(", ")}. Revise os cards e confirme sinais pendentes com especialista em Libras.`;
